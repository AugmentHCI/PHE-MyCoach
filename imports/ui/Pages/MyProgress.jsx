...
export default class MyProgress extends Component {
  ...
  fetchFitbitData(selectedPeriod, token) {
      ...
      const urlFit = `https://connector.idewe.be/healthempower/jobstudenten/api/fit-data?van=${selectedPeriod[0]}&tot=${selectedPeriod[1]}`;
  ...
  fetchFitbitDayData(date, token) {
    ...
    // Now try to fetch FitBit data as well
    if (typeof token !== 'undefined' && token !== "demo") {
      const urlFit = `https://connector.idewe.be/healthempower/jobstudenten/api/fit-data?van=${dateString}&tot=${dateString}`;
